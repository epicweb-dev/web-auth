...
Now, to be clear, let's try to make it impossible for an adversary to gain
access to your database, but we can reduce the attractiveness of this by making
it impossible for the attacker to use the passwords even if they do get access
to the database.
...
password they provide to bcrypt's `compare` function will verify the password is
correct.
